...
(* to ne deluje za negativna števila*)
...
let mapi f list = 
  (* let seznam = range ((List.length list) - 1) in*)
  let rec aux acc n = function
    | [] -> reverse acc
    | x::xs -> aux (f n x :: acc) (n + 1) xs
  in aux [] 0 list 
...
let rec zip l1 l2 = 
  ...
  | _ -> failwith "Seznama nista enake dolžine"
...
let unzip_tlrec _ = ()
(* zgornja funkcija je že repno rekruzivna *)
...
let apply_sequence f x n =
  let rec aux acc trenutni st = 
    if st < 0 then acc
    else aux (f trenutni :: acc) (f trenutni) (st - 1)
  in reverse (aux [x] x (n-1))
